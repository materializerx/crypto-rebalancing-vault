...
@notice Crypto Rebalancing Strategy Vault
*/
...
contract Vault is Ownable {
  ...
  // TODO: Change to Rinkeby network address
  /* Kovan Addresses */
  ...
  function getTotalBalance() public view returns(uint) {
    // check if oracle service has been called to update ETH price 
    ...
    // there are two functions for updating eth price
    // return prices are totally separated
    // this is useful for testing. we can adjust the portfolio and see how that affects the portfolio
  ...
  function updateEthPriceChainlink() public returns(uint) {
    ...
    // output will be in signed integer, so we need to convert to unsigned integer so it matches the function of uniswap's return value
  ...
  function buyWeth(uint amountUSD) internal {
    ...
    // 0.3, which is standard pool size
    uint24 fee = 3000;
    address recipient = address(this);
    uint256 amountIn = amountUSD; // includes 18 decimals
    // this would open up for slippage and front running if we were using for large transactions in mainnet
    // but no one's is front running, so that's fine
  ...
  function annualDividend() public onlyOwner {
    ...
    // this is optional, since it reduces the usefulness of the anualDividend function 
    // but can serve for testing purposes
